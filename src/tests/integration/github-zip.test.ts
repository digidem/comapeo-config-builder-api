...
describe('GitHub ZIP Integration Test', () => {
  const app = createApp();
  let zipFilePath: string;
  let tmpDir: string;

  // Download and save the GitHub ZIP file before running tests
  beforeAll(async () => {
    // Create a temporary directory
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'github-zip-test-'));
  ...
  it('should process a GitHub ZIP file and return a comapeocat file', async () => {
    // Read the ZIP file
